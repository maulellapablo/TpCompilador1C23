...
lista_var:		ID {guardarEnTablaSimbolos(TS_ID,$1); strcpy(matrizVariables[contadorId],yylval.strid) ;  contadorId++; contadorVar++;ptr_list_var = crearHoja($1);}
...
asignacion:		ID OPAR_ASIG expresion {validarSimbolo($1); validarTipoSimbolo($1, TS_INT); ptr_asig = crearNodo(":=", crearHoja($1), ptr_expr);}
				|ID OPAR_ASIG CTE_STRING {validarSimbolo($1); validarTipoSimbolo($1, TS_STRING); guardarEnTablaSimbolos(TS_STRING,$3); ptr_asig = crearNodo(":=", crearHoja($1), crearHoja($3)); _aux[_cont] = $1; };
...
factor:			ID {validarSimbolo($1);ptr_fact = crearHoja($1); _aux[_cont] = $1;}
				|CTE_ENTERA {guardarEnTablaSimbolos(TS_INT,$1); ptr_fact = crearHoja($1); _aux[_cont] = $1;}
				|CTE_REAL {guardarEnTablaSimbolos(TS_FLOAT,$1); ptr_fact = crearHoja($1); _aux[_cont] = $1;}
				|PAR_A expresion PAR_C
				|elementinthemiddle {ptr_fact = ptr_inmid; printf("Condicion ElementInTheMiddle - OK\n"); } ;
 
entrada: 		READ PAR_A ID PAR_C {validarSimbolo($3); ptr_entr = crearHoja($3);};

salida:			 WRITE CTE_STRING {guardarEnTablaSimbolos(TS_STRING,$2); ptr_sali = crearHoja($2);}
			    |WRITE PAR_A ID PAR_C {validarSimbolo($3); ptr_sali = crearHoja($3);}
				|WRITE PAR_A CTE_STRING PAR_C {guardarEnTablaSimbolos(TS_STRING,$3); ptr_sali = crearHoja($3);};
